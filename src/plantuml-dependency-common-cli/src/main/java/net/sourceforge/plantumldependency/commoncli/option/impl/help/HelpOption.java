...
/**
 * A default implementation managing the "-help" option, displaying the program usage information.
 ...
 * @version 1.3.0
 */
public class HelpOption extends AbstractOption implements ExecutableOption {

    /** Serial version UID. */
    private static final long serialVersionUID = 3929762825343812282L;

    /** The class logger. */
    private static final transient Logger LOGGER = getLogger(HelpOption.class.getName());

    /** Option main synopsis. */
    public static final String OPTION_MAIN_SYNOPSIS = "-h";

    /** Option synopsis alias. */
    public static final Set < String > OPTION_SYNOPSIS = unmodifiableSet(new TreeSet < String >(asList(new String[] {
            "--help", "-?"})));

    /** The java program to display help information. */
     ...
     * @since 1.3.0
     */
    public HelpOption(final JavaProgram program, final int optionPriority) {
        this(new StringBuilder("To display this help message."), program, optionPriority, ACTIVE_OPTIONAL_OPTION_STATUS);
    }
     ...
     * @since 1.3.0
     */
    public HelpOption(final StringBuilder fullOptionDescription, final JavaProgram program, final int optionPriority,
     ...
     */
    @Override
    public HelpOption deepClone() {
        final HelpOption a = (HelpOption) super.deepClone();
    ...
    public OptionExecution parseCommandLine(final CommandLine commandLine) throws CommandLineException {
        ...
        if (commandLine.isOptionActiveAndSpecified(this)) {
            optionExecution = new HelpOptionExecution(getJavaProgram(), getPriority());
        } else {
